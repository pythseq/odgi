#ifndef ODGI_REGION_HPP
#define ODGI_REGION_HPP
...
namespace odgi {
    ...
    inline void parse_region(std::string &region, Region &out_region) {
        parse_region(region, out_region.seq, out_region.start, out_region.end);
    }
    ...
    void parse_bed_regions(const std::string &bed_path, std::vector<Region> &out_regions,
                           std::vector<std::string> *out_names = nullptr);

}

#endif //ODGI_REGION_HPP
